...
flowchart TD
    ...
    FaultReport -->|"resulting status = broken<br>+ needs_repair not present"| AddRepair["+needs_repair"]
    FaultReport -->|"needs_investigation not present"| AddInv["+needs_investigation"]

    Assessment -->|"resulting status = broken<br>+ needs_repair not present"| AddRepair2["+needs_repair"]
...
|---|---|---|---|
| `fault_report` | `unknown` | `needs_investigation` not present | suggest `+needs_investigation` |
| `fault_report` | `unknown` | `needs_investigation` present | — no change — |
| `fault_report` | `broken` | `needs_repair` not present, `needs_investigation` not present | suggest `+needs_repair`, `+needs_investigation` |
| `fault_report` | `broken` | `needs_repair` not present, `needs_investigation` present | suggest `+needs_repair` |
| `fault_report` | `broken` | `needs_repair` present, `needs_investigation` not present | suggest `+needs_investigation` |
| `fault_report` | `broken` | `needs_repair` present, `needs_investigation` present | — no change — |
| `fault_report` | `working` | `needs_investigation` not present | suggest status → `unknown`, `+needs_investigation` |
| `fault_report` | `working` | `needs_investigation` present | suggest status → `unknown` |
| `assessment` | `unknown` | — | — no change — |
| `assessment` | `broken` | `needs_repair` not present | suggest `+needs_repair` |
| `assessment` | `broken` | `needs_repair` present | — no change — |
...
| `cleaning` | `broken` | `needs_cleaning` not present | — no change — |
| `cleaning` | `working` | `needs_cleaning` present | suggest `-needs_cleaning` |
| `cleaning` | `working` | `needs_cleaning` not present | — no change — |
| `other` | any | — | — no change — |
